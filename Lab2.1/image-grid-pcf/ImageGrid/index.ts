...
export class ImageGrid implements ComponentFramework.ReactControl<IInputs, IOutputs> {
  ...
  public updateView(context: ComponentFramework.Context<IInputs>): React.ReactElement {
    ...
    if (datasetChanged || isTestHarness) {
      ...
      let imageValueColumn = dataset.columns.find(column => column.alias === "ImageContent" || column.dataType === "Image")?.name;

      // In Power Pages the column data type is a string not Image
      if (!imageValueColumn) imageValueColumn = dataset.columns.find(column => (column as any).attributes?.AttributeTypeName?.Value === "ImageType")?.name;

      if (imageValueColumn) {
        // Convert the dataset records into the ISliderPhoto interface
        this.images = dataset.sortedRecordIds.map((id) => {
          const record = dataset.records[id];
          const imageValue = imageValueColumn != null ? record.getValue(imageValueColumn) as any : undefined;
          let fileContent = ''
    ...
    return React.createElement(
      ...
      height: modelDrivenAppSubGridHeight !== null && modelDrivenAppSubGridHeight > 0 ? modelDrivenAppSubGridHeight : allocatedHeight,
      images: this.images
    }
    );
